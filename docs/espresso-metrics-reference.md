...
- **Dashboard panel** - whether a panel exists in the Grafana dashboard (`📊 Panel` = has dedicated panel, `📋 Reference only` = metric available but no panel in dashboard)
...
**Dashboard:** 📊 Panel - *Block Height / Block Height Over Time / DA Scanner Progress vs Chain Tip*  
...
**Dashboard:** 📊 Panel - *Node Index*  
**Description:** The index assigned to this node within the current active validator set. A value of `-1` or `0` may indicate the node is not yet registered in the active stake table for the current epoch.
...
**Dashboard:** 📊 Panel - *Current View / Current View vs Last Decided View / Last Voted View vs Current View*  
...
**Dashboard:** 📊 Panel - *Last Decided View / Current View vs Last Decided View*  
...
**Dashboard:** 📊 Panel - *Last Voted View / Last Voted View vs Current View*  
...
**Type:** Counter  
**PromQL:** `increase(consensus_number_of_timeouts{job=~"$node"}[5m])`  
**Dashboard:** 📊 Panel - *Total Timeouts / Timeouts Rate*  
...
**Dashboard:** 📊 Panel - *Views Per Decide Event*  
...
**Dashboard:** 📊 Panel - *Views Since Last Decide*  
**Description:** Number of views that have elapsed since the last block was decided. Under normal conditions this should stay low (1-5). A large and growing value is an early warning that the network has stalled and no new blocks are being finalized.
...
**Dashboard:** 📊 Panel - *Proposal to Decide Time*  
...
**Dashboard:** 📊 Panel - *Outstanding Transactions*  
...
**Dashboard:** 📊 Panel - *Connected Peers / Connected Peers Over Time*  
...
**Dashboard:** 📊 Panel - *libp2p Ready*  
**Description:** Whether the libp2p networking layer has completed initialization and is ready to communicate with peers. Should be `1` shortly after node startup. If it stays at `0`, there is a networking configuration problem (port not open, advertise address incorrect, etc.).
...
**Dashboard:** 📊 Panel - *CDN Failed Messages*  
...
**Dashboard:** 📊 Panel - *CliqueNet Sequencer Connections*  
...
**Dashboard:** 📊 Panel - *Internal Event Queue Length*  
...
**Dashboard:** 📊 Panel - *L1 Head vs L1 Finalized*  
...
**Dashboard:** 📊 Panel - *L1 Failed Requests & Failovers*  
...
**Dashboard:** 📊 Panel - *L1 Failed Requests & Failovers / L1 Failovers Over Time / L1 Failovers (1h)*  
...
**Dashboard:** 📊 Panel - *L1 Stream Reconnects / L1 Stream Reconnects Over Time*  
...
**Dashboard:** 📊 Panel - *DA Scanner Running*  
...
**Dashboard:** 📊 Panel - *Scanned Blocks (DA)*  
...
**Dashboard:** 📊 Panel - *Missing Blocks (DA) / DA Scanner - Missing Blocks & VID*  
...
**Dashboard:** 📊 Panel - *Missing VID (DA) / DA Scanner - Missing Blocks & VID*  
...
**Dashboard:** 📊 Panel - *Scanner Current Block / DA Scanner Progress vs Chain Tip*  
**Description:** The block height currently being scanned by the DA scanner. Compare against `aggregator_height` to determine how far behind the scanner is relative to the chain tip. A large lag is normal during initial sync but should converge over time.
...
**Dashboard:** 📊 Panel - *SQL Open Transactions*  
...
**Dashboard:** 📊 Panel - *SQL Transactions*  
...
**Dashboard:** 📊 Panel - *Proposal Fetcher Queue*  
...
**Dashboard:** 📊 Panel - *Proposal Fetcher - Last Fetched vs Last Seen*  
...
**Dashboard:** 📊 Panel - *Catchup Requests (5m) / Catchup Requests & Failures*  
...
**Dashboard:** 📊 Panel - *Catchup Failures (5m) / Catchup Requests & Failures*  
...
**Dashboard:** 📋 Reference only  
**Description:** Whether the proactive sync scanner is running. This background process proactively scans for missing data ranges and fetches them from peers. May require specific node configuration to activate.
...
**Dashboard:** 📋 Reference only  
**Description:** Number of block ranges that have been scanned for missing data. Increases during sync and should stabilize once the node is fully caught up. May require specific node configuration to activate.
...
**Dashboard:** 📋 Reference only  
**Description:** Average time in milliseconds to scan one block for missing data. A useful indicator of sync performance - high values indicate the scanner is slow, possibly due to database or network bottlenecks. May require specific node configuration to activate.
...
**Dashboard:** 📋 Reference only  
**Description:** Size of the block ranges being scanned in each sync iteration. The node dynamically adjusts this based on available resources and network conditions. May require specific node configuration to activate.

---

## 🧮 Derived Metrics & PromQL Recipes

The following expressions are not single metrics but are used in the Grafana dashboard as computed indicators. They are documented here as reference for custom alerting or additional panels.
...
**Dashboard:** 📊 Panel - *Scanner Lag (blocks)*  
**Description:** The number of blocks the DA scanner is behind the current chain tip. Computed as the difference between `aggregator_height` (current chain tip) and `scanner_current` (last block scanned). A lag of a few hundred blocks is normal during active syncing. A lag that keeps growing without converging means the scanner cannot keep up with the chain - investigate storage or network bottlenecks. Should reach near zero on a fully synced node.

---

### Timeout rate (5m window)

```promql
increase(consensus_number_of_timeouts{job=~"$node"}[5m])
```

**Description:** Rate of view timeouts over a rolling 5-minute window. More actionable than the raw counter for alerting - set alert thresholds based on your network's observed baseline.

---

### L1 head/finalized lag

```promql
consensus_l1_head{job=~"$node"} - consensus_l1_finalized{job=~"$node"}
```

**Description:** Gap between the latest L1 block seen and the latest finalized block. Under normal Ethereum conditions this should be ~64 blocks. Significantly larger values indicate L1 reorganization activity or a delayed finality signal from the RPC provider.

---

### Catchup failure ratio

```promql
increase(consensus_catchup_request_failures{job=~"$node"}[5m])
/
clamp_min(increase(consensus_catchup_requests{job=~"$node"}[5m]), 1)
```

**Description:** Ratio of failed to total catchup requests. Values above 0.5 (50% failure rate) indicate the node is struggling to sync from peers. Useful as an alert condition during the post-restart sync window.
